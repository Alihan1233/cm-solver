...
// Функция для вывода компонента в консоль
...
Component parseComponent(cJSON* componentJSON) {
    ...
    safeStrCpy(component.compound, sizeof(component.compound), cJSON_GetObjectItem(componentJSON, "Compound")->valuestring);
    component.mol = cJSON_GetObjectItem(componentJSON, "Mol")->valuedouble;
    component.molWeight = cJSON_GetObjectItem(componentJSON, "MolWeight")->valuedouble;
    component.density = cJSON_GetObjectItem(componentJSON, "Density")->valueint;
    return component;
}
...
PtvTablePoint parsePtvTablePoint(cJSON* ptvTablePointJSON) {
    ...
    safeStrCpy(ptvTablePoint.id, sizeof(ptvTablePoint.id), cJSON_GetObjectItem(ptvTablePointJSON, "Id")->valuestring);
...
Data parseData(const char* jsonString) {
    ...
    if (root != NULL) {
        safeStrCpy(data.id, sizeof(data.id), cJSON_GetObjectItem(root, "Id")->valuestring);
        safeStrCpy(data.name, sizeof(data.name), cJSON_GetObjectItem(root, "Name")->valuestring);
        safeStrCpy(data.type, sizeof(data.type), cJSON_GetObjectItem(root, "Type")->valuestring);
        safeStrCpy(data.reservoir, sizeof(data.reservoir), cJSON_GetObjectItem(root, "Reservoir")->valuestring);
        data.phases = cJSON_GetObjectItem(root, "Phases")->valueint;
        safeStrCpy(data.equationOfState, sizeof(data.equationOfState), cJSON_GetObjectItem(root, "EquationOfState")->valuestring);
